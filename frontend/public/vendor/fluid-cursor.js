...
class FluidCursor {
    ...
    /**
     * Элементы с data-fluid-cursor-block — «контент»: эффект не реагирует (только «пустые» зоны).
     */
    isFluidCursorBlocked(clientX, clientY) {
        if (typeof document.elementFromPoint !== 'function') return false;
        const top = document.elementFromPoint(clientX, clientY);
        if (!top || !(top instanceof Element)) return false;
        return Boolean(top.closest('[data-fluid-cursor-block]'));
    }
    ...
    initEvents() {
        ...
        window.addEventListener('mousedown', (e) => {
            if (this.isFluidCursorBlocked(e.clientX, e.clientY)) return;
